...
public class LdapConnection {
    ...
    private Pair<AuthenticationResult, LdapContext> createContext(String principal, GuardedString credentials) {
        ...
        if (config.isStartTls()) {
            ...
            env.put(Context.SECURITY_AUTHENTICATION, getAuthType(principal));
    ...
    private Pair<AuthenticationResult, LdapContext> createContext(final Hashtable<?, ?> env) {
        ...
        try {
            context = new InitialLdapContext(env, null);
            authnResult = getPasswordExpiredResult(context);
        } catch (AuthenticationException e) {
            authnResult = getResultFromAuthException(e);
        } catch (NamingException e) {
    ...
    private AuthenticationResult authenticateContext(final InitialLdapContext context, String principal,
        ...
        AuthenticationResult authnResult = null;
    	try {    		            
	    	context.addToEnvironment(Context.SECURITY_AUTHENTICATION, getAuthType(principal));	
	        if (isNotBlank(principal)) {
    ...
    private String getAuthType(String principal){
        if (SASL_GSSAPI.equalsIgnoreCase(config.getAuthType())) {
            return "GSSAPI";
        } 
        return isNotBlank(principal) ? "simple" : "none";
    }
   
    private AuthenticationResult getResultFromAuthException(AuthenticationException e){
        AuthenticationResult authnResult = null;
    ...
    private AuthenticationResult getPasswordExpiredResult(InitialLdapContext context) throws NamingException{        
        if (config.isRespectResourcePasswordPolicyChangeAfterReset()) {
            if (hasPasswordExpiredControl(context.getResponseControls())) {
                return new AuthenticationResult(AuthenticationResultType.PASSWORD_EXPIRED);
            }
        }
        return null;
    }
